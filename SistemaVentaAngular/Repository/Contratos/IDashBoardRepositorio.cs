namespace SistemaVentaAngular.Repository.Contratos
{
    public interface IDashBoardRepositorio
    {
        ...
        Task<string> TotalPiadIngresosUltimaSemanaFilter(string filtertype, string? startdate, string? enddate);
        Task<int> TotalProductosFilter(string filtertype, string? startdate, string? enddate);
        Task<int> TotalVentasCreditoFilter(string filtertype, string? startdate, string? enddate);
    }
}
